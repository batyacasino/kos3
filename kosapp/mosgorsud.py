...
def statemovements_db_creat(client, data):
	...
	statemovements_date = StateMovement.objects.filter(date=date_formate)
	...
	StateMovement.objects.create(
								...
								date = date_formate,
								condition = data[0],
								document_base = data[1]
								)


def courtsessions_db_creat(client, data):
	date_formate = date_format(data[0])
	courtsessions_date = CourtSessions.objects.filter(date=date_formate)
...
def judicialacts_db_creat(client, data):
	date_formate = date_format(data[0])
	judicialacts_date = JudicialActs.objects.filter(date=date_formate)
